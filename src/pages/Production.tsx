...
const schema = z.object({
  ...
  ordem_producao: z.string().min(1, 'Ordem de Produção (OP) é obrigatória'),
  ...
  items: z
    .array(
      z.object({
        item_id: z.string().min(1, 'Selecione uma ferramenta'),
        quantidade: z.coerce.number().min(1, 'Quantidade deve ser maior que 0'),
      }),
    )
    .min(1, 'Adicione pelo menos uma ferramenta'),
})
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      for (const item of data.items) {
        ...
        if (!dbItem) {
          toast.error('Ferramenta inválida selecionada')
          return
        }
        if (item.quantidade > dbItem.quantidade_atual) {
          toast.error(
            `Quantidade indisponível para a ferramenta ${dbItem.nome}. Saldo: ${dbItem.quantidade_atual}`,
          )
          return
        }
      }
      ...
      toast.success('Saída de ferramentas registrada com sucesso!')
      navigate('/estoque')
    } catch (error: any) {
      console.error(error)
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-10">
      <div className="flex items-center gap-4 border-b pb-4">
        ...
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Saída de Estoque FERRAMENTAS</h2>
          <p className="text-muted-foreground">
            Registre a retirada de múltiplas ferramentas do estoque para produção.
          </p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Ex: OP-2023-001" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle>Ferramentas da Requisição</CardTitle>
                <CardDescription>
                  Adicione as ferramentas que serão retiradas do estoque.
                </CardDescription>
              </div>
              <Button
                type="button"
